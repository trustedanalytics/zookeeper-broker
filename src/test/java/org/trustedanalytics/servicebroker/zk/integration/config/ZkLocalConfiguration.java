...
public class ZkLocalConfiguration {
  ...
  public TestingServer initEmbededZKServer() throws Exception {
    ...
    ZooKeeperTestOperations.createSecuredNode(credendials, brokerRootNode);
    return zkServer;
  }

  @Bean(initMethod = "init", destroyMethod = "destroy")
  @Qualifier(org.trustedanalytics.servicebroker.framework.Qualifiers.BROKER_STORE)
  ...
  @Profile("integration-test")
  public ZookeeperClient getInstanceZkClient(TestingServer zkServer) throws IOException {
    ZookeeperClient client =
        new ZookeeperClientBuilder(zkServer.getConnectString(), user, password, brokerRootNode)
            .build();
    return client;
  }
}
